...
const VOCABULARY: Word[] = [
  ...
  { en: "iomanip", cn: "格式化输出头文件", category: "基础语法", phonetic: "/ˌaɪoʊˈmænɪp/", example: "Input/output manipulators." },
  { 
    en: "int", 
    cn: "整型", 
    category: "数据类型", 
...
export default function App() {
  ...
  const [appMode, setAppMode] = useState<"study" | "quiz" | "spell">("study");
  ...
  const generateSpellQuestion = () => {
    const list = filteredVocab.length > 0 ? filteredVocab : VOCABULARY;
  ...
  const checkSpell = () => {
    ...
    if (isCorrect) {
      ...
      // Don't auto-next on fail so they can see/fix it or ask for answer
    }
  };
  ...
  const generateQuizQuestion = () => {
    ...
    const others = VOCABULARY.filter(w => w.en !== randomWord.en);
  ...
  const categories = useMemo(() => {
    return ["全部", "我的收藏", ...Array.from(new Set(VOCABULARY.map(w => w.category)))];
  }, []);
  ...
  useEffect(() => {
    ...
    setIsFlipped(false);
    setAccuracyScore(null);
  }, [selectedCategory, searchQuery, filteredVocab.length]);
  ...
  const isFavorite = favorites.some(w => w.en === currentWord.en);
  ...
  return (
    ...
    >
        ...
        <AnimatePresence mode="wait">
          {filteredVocab.length === 0 ? (
            ...
            >
              ...
              <h3 className="text-xl font-bold text-olive">没有找到匹配的结果</h3>
              <p className="text-sm text-olive/50 mt-2 mb-8 leading-relaxed">
                未能找到包含 "{searchQuery}" 的单词。<br/>请尝试更换关键词，或切换分类。
              </p>
              <button
                onClick={() => { playSound('click'); setSearchQuery(""); setSelectedCategory("全部"); }}
            ...
            >
              <div className="relative w-full aspect-[3/2] perspective-1000 mb-12">
                <AnimatePresence mode="wait" initial={false}>
                  ...
                  >
                    {/* Front Side */}
                    <div className="absolute inset-0 backface-hidden bg-card-front rounded-[24px] shadow-[0_20px_40px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_40px_rgba(0,0,0,0.4)] border border-card-border flex flex-col p-10 items-center justify-center text-center">
                    ...
                    {/* Back Side */}
                    <div 
                      className="absolute inset-0 backface-hidden bg-card-back rounded-[24px] shadow-[0_20px_40px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_40px_rgba(0,0,0,0.4)] border border-card-border flex flex-col p-10 items-center justify-center text-center"
                    ...
                    >
                      ...
                      <div className="mt-auto pt-6 border-t border-[#e0dfd5]/50 w-full text-sm text-[#a1a196] italic">
                        "Keep learning C++"
                      </div>
                    </div>
                  </motion.div>
              ...
              <div className="flex items-center gap-4">
                ...
                >
                  <span>随机切换</span>
                  <Shuffle className="h-5 w-5 font-bold" />
                </motion.button>
            ...
            >
              ...
              <div className="w-full flex justify-between items-center mb-8 px-4">
                <div className="flex items-center gap-2 text-olive font-bold">
                  ...
                  <span>正确率: {quizTotal > 0 ? Math.round((quizScore / quizTotal) * 100) : 0}% ({quizScore}/{quizTotal})</span>
                </div>
            ...
            >
              ...
              <div className="w-full flex justify-between items-center mb-8 px-4">
                <div className="flex items-center gap-2 text-olive font-bold">
                  ...
                  <span>掌握进度: {spellScore} | 练习总数: {spellTotal}</span>
                </div>
                <div className="text-xs font-bold uppercase tracking-widest text-[#a1a196] bg-card-front/50 px-3 py-1 rounded-full border border-card-border">
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-lg bg-card-front rounded-[32px] shadow-2xl overflow-hidden flex flex-col max-h-[85vh] border border-card-border"
            >
              <div className="p-8 border-b border-card-back flex items-center justify-between bg-natural-bg/30">
                ...
                <button 
                  onClick={() => { playSound('click'); setIsModalOpen(false); }}
                  className="text-[#a1a196] hover:text-olive transition-colors underline text-sm font-medium"
                >
                  关闭
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                {favorites.length === 0 ? (
                  <div className="h-64 flex flex-col items-center justify-center text-[#a1a196] opacity-60">
                    <BookOpen className="h-16 w-16 mb-4 stroke-1" />
                    <p className="font-medium italic">你的生词本还是空的</p>
                  </div>
                ) : (
                  <div className="divide-y divide-card-back">
                    {favorites.map((word) => (
                      <motion.div 
                        layout
                        key={word.en}
                      ...
                      >
                        <div>
                          <div className="font-bold text-olive text-lg">{word.en}</div>
                          <div className="text-accent-gold text-sm font-medium italic">{word.cn}</div>
                        </div>
                        <div className="flex items-center gap-3">
                           <span className="hidden sm:inline text-[10px] font-bold text-[#a1a196] uppercase tracking-widest">
                            {word.category}
                           </span>
                           <button 
                            onClick={() => removeFavorite(word.en)}
                            className="text-heart-active/60 hover:text-heart-active underline text-xs font-semibold"
                          >
                            移除
                          </button>
                        </div>
              ...
              {favorites.length > 0 && (
                <div className="p-6 bg-natural-bg border-t border-card-back flex justify-center">
                   <button 
                    onClick={() => {
                      playSound('click');
                    ...
                    className="text-[#a1a196] hover:text-olive text-xs font-bold uppercase tracking-widest transition-colors"
                  >
                    清空列表
                  </button>
                </div>
      ...
      <footer className="mt-auto py-8 text-[#a1a196] text-sm italic">
        Mastering {VOCABULARY.length} C++ terms • Keep Growing
      </footer>
    </div>
  );
}
